...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="cora",
                        choices=['cora', "pubmed", "citeseer", "wikics", "arxiv", "instagram", "reddit", "computer",
                                 "photo", "history", "cornell", "wisconsin", "texas", "washington"])
    parser.add_argument("--device", type=str, default="cuda:0")
    parser.add_argument("--encoder_name", type=str, default="SentenceBert",
                        choices=["MiniLM", "SentenceBert", "e5-large", "roberta", "Qwen-3B", "Qwen-7B", "Mistral-7B",
                                 "Llama-8B"])
    ...
    graph_data = torch.load(f"{args.root}/{args.dataset}.pt", weights_only=False, map_location=torch.device('cpu'))

    print('= ' * 20)
    print('## Starting Time:', get_cur_time(), flush=True)
    print(args, "\n")

    if os.path.exists(f"{args.root}/{args.encoder_name}/{args.dataset}.pt"):
    ...
    with torch.no_grad():
        text_embeddings = []
        all_texts = graph_data.raw_texts
        for i in tqdm(range(0, len(all_texts), args.batch_size), desc=f"Generating {encoder_type} Embedding"):
            batch_texts = all_texts[i:i + args.batch_size]
            batch_emb = text_encoder(input_text=batch_texts, pooling="cls" if args.use_cls else "mean")
    ...
    if args.save_emb:
        write_dir = f"{args.root}/{args.encoder_name}"
