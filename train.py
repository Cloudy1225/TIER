...
from utils import array_mean_std, compute_acc_and_f1, set_seed
from gnns import GNNEncoder
...
def gnn_train(gnn_model, graph_data, sample_cluster_indices, cluster_hierarchy, tree_pairwise_dist, lamda):
    gnn_model.train()
    optimizer.zero_grad()
    output, z = gnn_model(graph_data.x, graph_data.edge_index, return_embeddings=True)
    loss_ce = F.cross_entropy(output[graph_data.train_mask], graph_data.y[graph_data.train_mask])

    labels = torch.from_numpy(sample_cluster_indices[:, -1]).to(z.device)
    n_clusters = int(max(labels)) + 1
    centroids = torch.zeros((n_clusters, z.shape[1]), dtype=z.dtype, device=z.device)
    counts = torch.bincount(labels, minlength=n_clusters).view(-1, 1)
    centroids.index_add_(0, labels, z)
    centroids = centroids / counts
    centroids = F.normalize(centroids, dim=-1, p=2)

    pairwise_dist = F.pdist(centroids, p=2)

    pairwise_dist = pairwise_dist - pairwise_dist.mean()
    tree_pairwise_dist = tree_pairwise_dist - tree_pairwise_dist.mean()

    numerator = torch.sum(pairwise_dist * tree_pairwise_dist)
    denominator = torch.norm(pairwise_dist, p=2) * torch.norm(tree_pairwise_dist, p=2)
    loss_ccc = 1. - numerator / denominator
    loss = loss_ce + lamda * loss_ccc

    loss.backward()
    optimizer.step()
    return float(loss), float(loss_ce), float(loss_ccc)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_clusters_list', type=str, default="6, 64", )
    # Encoder
    ...
    parser.add_argument("--gnn_type", type=str, default="GCN",
                        choices=["GCN", "GAT", "SAGE", "TransformerConv"])
    ...
    args.n_clusters_list = [int(n_clusters.strip()) for n_clusters in args.n_clusters_list.split(",")]
    ...
    if args.write_result:
        os.makedirs("./results/TIRAG", exist_ok=True)
        write_file = open(f"./results/TIRAG/{args.dataset}{'' if not args.re_split else '_s'}.csv", mode='a',
                          newline='')
    ...
    for i in range(args.run_times):
        ...
        n_finest_clusters = args.n_clusters_list[-1]
        save_path = os.path.join('taxonomy', 'outputs', args.dataset,
                                 str(n_finest_clusters), f'hierarchical_clustering_'
                                       f'{"_".join([str(n_clusters) for n_clusters in args.n_clusters_list])}.joblib')
        sample_cluster_indices, cluster_hierarchy, tree_pairwise_dist = joblib.load(save_path)
        ...
        print("tree_pairwise_dist shape:", tree_pairwise_dist.shape)
        tree_pairwise_dist = torch.tensor(tree_pairwise_dist, device=device, dtype=torch.float32)
        ...
        for epoch in range(1, args.epochs + 1):
            loss, loss_ce, loss_ccc = gnn_train(gnn_model, graph_data, sample_cluster_indices,
                                                cluster_hierarchy, tree_pairwise_dist, args.lamda)
            [train_acc, val_acc, test_acc], [train_mac_f1, val_mac_f1, test_mac_f1], [train_weight_f1, val_weight_f1,
                                                                                      test_weight_f1] = gnn_test()
            ...
            if epoch % args.print_freq == 0:
                print(
                    f"Epoch {epoch:03d}   Loss={loss:.4f} CE={loss_ce:.4f} CCC={loss_ccc:.4f}  "
                    f"Train acc {train_acc:.3f} Val acc {val_acc:.3f} Test acc {test_acc:.3f}  "
                    f"Train F1 {train_mac_f1:.3f} Val F1 {val_mac_f1:.3f} Test F1 {test_mac_f1:.3f}")
        ...
        print(
            f'[Times {i}] Test Acc {best_test_acc:.2f}  Test Macro-F1 {best_test_mac_f1:.2f}  Test Micro-F1 {best_test_weight_f1:.2f} Time {timer_list[-1]:.3f}s\n')
    ...
    if args.write_result:
        ...
        writer.writerow(
            [args.gnn_type, args.n_layers, args.hidden_dim, args.dropout, args.encoder_name, args.batch_norm,
             args.residual_conn, args.n_clusters_list, args.lamda, f"{acc_mean:.2f}±{acc_std:.2f}", f"{macrof1_mean:.2f}±{macrof1_std:.2f}",
             f"{weightf1_mean:.2f}±{weightf1_std:.2f}", trainable_params, f"{sum(timer_list) / len(timer_list):.3f}s"])
