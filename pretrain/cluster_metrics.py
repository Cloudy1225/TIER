...
def _hungarian_match(y_true: np.ndarray, y_pred: np.ndarray) -> np.ndarray:
    ...
    for i, true_label in enumerate(np.unique(y_true)):
        for j, pred_label in enumerate(np.unique(y_pred)):
            cost_matrix[i, j] = np.sum((y_true == true_label) & (y_pred == pred_label))  # Maximize overlap

    # Use the Hungarian algorithm (linear sum assignment) to find the optimal label mapping
    row_ind, col_ind = linear_sum_assignment(-cost_matrix)  # Negate cost to maximize overlap

    # Create label map and align predicted labels
...
def label_metrics(
    ...
    Returns:
        If a single metric is requested, returns a single float.
        If multiple metrics are requested, returns a tuple of floats.
    """
    ...
    # Return either a single metric or a tuple of metrics
    if len(metrics) == 1:
        return results[metrics[0]]
    else:
        return tuple(results[m] for m in metrics)
